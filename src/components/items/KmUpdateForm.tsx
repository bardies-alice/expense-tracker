...
export function KmUpdateForm({ itemId, categoryId, currentKm }: { itemId: string; categoryId: string; currentKm: number }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex items-center gap-2.5">
      <label className="text-xs font-medium uppercase tracking-wide text-muted">Kilometraje</label>
      <div className="w-32">
      ...
      <button
        type="submit"
        disabled={pending}
        className="h-10 rounded-lg border border-ink px-5 text-sm font-medium text-ink hover:bg-ink/5 disabled:opacity-50"
      >
        {pending ? "..." : "Actualizar"}
      </button>
    </form>
